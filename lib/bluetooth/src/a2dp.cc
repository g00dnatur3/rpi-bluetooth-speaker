...
namespace a2dp {
	...
	void decode(sbc_args_t *sbc_args) {
		...
		sbc_args->in_buf_len = sbc_args->bytes_read -= RTP_HEADER_SIZE;
		//printf("    sbc_args->bytes_read=%d\n", sbc_args->bytes_read);
		size_t total_written = 0;
		for (;;) {
			if (sbc_args->in_buf_len == 0) break; //gracfeul
			if (sbc_args->in_buf_len < 0) {
				printf("a2dp: Error while decoding, in_buf_len < 0 \n");
				break;
			}
	        int written = 0;
	        ...
	        //printf("  decode() - in_buf_len = %d \n", sbc_args->in_buf_len);
	        //printf("  decode() - out_buf_len = %d \n", sbc_args->out_buf_len);
	        //printf("  decode() - decoded = %d \n", decoded);
	        //printf("  decode() - written = %d \n", written);
	        //printf("\n");
	        if (decoded > 0) {
