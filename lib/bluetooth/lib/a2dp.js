...
const a2dp = require('../build/Release/a2dp');
...
a2dp.play = function(path, fd, mtu, config) {
	...
	console.log();
	console.log('a2dp.play() fd: ' + fd + ' config: ' + config + ' mtu: ' + mtu);
	console.log();

	const aplayOptions = ['-f', 'S16_LE', '-c', '2', '-r', '44100', '-t', 'raw', '--buffer-size', '4096'];
	const aplay = spawn('aplay', aplayOptions);
	...
	const loop = function() {
		a2dp.poll(fd, POLLIN, 10, function(pval) {
